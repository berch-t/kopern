...
import { apiReferenceMarkdown } from "@/components/docs/api-reference-content";
import { apiReferenceMarkdownFr } from "@/components/docs/api-reference-content-fr";
import { MarkdownRenderer } from "@/components/shared/MarkdownRenderer";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Download, Code2, Search, X } from "lucide-react";
...
export default function ApiReferencePage() {
  ...
  const content = locale === "fr" ? apiReferenceMarkdownFr : apiReferenceMarkdown;
  ...
  const handleSaveMarkdown = () => {
    ...
    a.href = url;
    a.download = "kopern-api-reference.md";
    a.click();
    URL.revokeObjectURL(url);
    toast.success(locale === "fr" ? "Fichier Markdown sauvegarde" : "Markdown file saved");
  };
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-0 -mx-6 -mt-6">
      ...
      <aside className="hidden xl:flex w-64 shrink-0 border-r flex-col overflow-y-auto py-6 pr-6 pl-4">
        {mounted && <TableOfContents contentRef={scrollRef} contentKey={`api-ref-${locale}`} />}
      </aside>
        ...
        <div className="mx-auto max-w-5xl px-6 py-6">
            ...
            <div className="mb-6">
              <div className="flex items-center gap-2 justify-end mb-3">
                <Button variant="outline" size="sm" onClick={handleSaveMarkdown}>
                  ...
                  {locale === "fr" ? "Sauvegarder .md" : "Save .md"}
                </Button>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-3xl font-bold">
                    {locale === "fr" ? "Reference API" : "API Reference"}
                  </h1>
                  <p className="text-muted-foreground">
                    {locale === "fr"
                      ? "Documentation complete de l'API Kopern"
                      : "Complete Kopern API documentation"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="relative mb-8 max-w-2xl" data-search-container>
            ...
            <Input
              ...
              onKeyDown={onKeyDown}
              placeholder={locale === "fr" ? "Rechercher dans la reference API..." : "Search API reference..."}
            ...
            {open && (
              <div className="absolute z-50 top-full mt-1 left-0 right-0 rounded-lg border bg-popover shadow-lg overflow-hidden">
                ...
                <div className="flex items-center justify-between px-4 py-1.5 border-t bg-muted/30 text-[11px] text-muted-foreground">
                  <span>{hits.length} {hits.length === 1 ? (locale === "fr" ? "resultat" : "result") : (locale === "fr" ? "resultats" : "results")}</span>
