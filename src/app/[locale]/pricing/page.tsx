    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="flex items-center px-4 md:px-6 py-4 max-w-6xl mx-auto">
        ...
        <Sheet>
          ...
          <SheetContent side="left" className="w-72 p-0">
            ...
            <nav className="flex flex-col gap-1 p-4 overflow-y-auto">
              ...
              {!loading && user && (
                <LocalizedLink href="/dashboard"><Button variant="outline" className="w-full mt-2 gap-2"><LayoutDashboard className="h-4 w-4" />{t.landing.ctaDashboard}</Button></LocalizedLink>
              )}
            </nav>
          </SheetContent>
        </Sheet>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <LocaleSwitcher />
          {loading ? (
            <div className="h-9 w-24 animate-pulse rounded-md bg-muted hidden md:block" />
          ) : user ? (
