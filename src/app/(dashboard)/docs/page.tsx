...
export default function DocsPage() {
  ...
  const { user } = useAuth();
  const tocItems = useMemo(() => extractToc(docsMarkdown), []);

  const handleSaveMarkdown = () => {
    const blob = new Blob([docsMarkdown.trim()], { type: "text/markdown" });
    ...
    toast.success("Documentation saved as Markdown");
  };
  ...
  const handleSaveAsSkill = async () => {
    ...
    const skillContent = docsMarkdown.trim();
    await navigator.clipboard.writeText(skillContent);
    toast.success("Documentation copied — paste it as a skill's content");
  };
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Complete guide to building agents with Kopern
                  </p>
                </div>
              </div>
